...
var destinationNamesWithin500Kms = travelDestinations
  .filter(travelDestination)
  .map(destination => destination.destinationName);
// Complete here

var destinationNameReachableByFerry = travelDestinations.find(transportation =>
  transportation.transportations.includes("ferry")
).destinationName;
// Complete here

function travelDestinationTrain(transportation) {
  return (
    transportation.distanceKms > 300 &&
    transportation.transportations.includes("train")
  );
}

var destinationNamesMoreThan300KmsAwayByTrain = travelDestinations
  .filter(travelDestinationTrain)
